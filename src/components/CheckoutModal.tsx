...
export const CheckoutModal: React.FC<CheckoutModalProps> = ({
  ...
  // Different Recipient / Delivery Destination
  ...
  const [scheduleTime, setScheduleTime] = useState('Pagi Ini (08:00 - 10:00)');
  const [customScheduleNotes, setCustomScheduleNotes] = useState('');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!customerName.trim()) {
      setErrorMessage('Mohon isi nama lengkap Anda (pemesan).');
      return;
    }

    const cleanPhone = customerPhone.replace(/\D/g, '');
    if (!cleanPhone || cleanPhone.length < 9) {
      setErrorMessage('Mohon isi nomor WhatsApp pemesan yang aktif dan valid (min. 10 digit).');
      return;
    }

    if (deliveryType === 'DELIVERY' && !deliveryAddress.trim()) {
      setErrorMessage('Mohon isi alamat pengiriman lengkap.');
      return;
    }

    if (deliveryType === 'DELIVERY' && isDifferentRecipient && !recipientName.trim()) {
      setErrorMessage('Mohon isi nama penerima di alamat pengiriman.');
      return;
    }
    ...
    try {
      ...
      const finalScheduleText = scheduleType === 'ASAP' 
        ? 'Secepatnya (Langsung Diproses)' 
        : (customScheduleNotes.trim() ? `${scheduleTime} - Catatan: ${customScheduleNotes.trim()}` : scheduleTime);

      let finalAddress = deliveryType === 'DELIVERY' ? deliveryAddress.trim() : 'Ambil di Toko (Pick-Up)';
      if (deliveryType === 'DELIVERY' && isDifferentRecipient && recipientName.trim()) {
        finalAddress = `[Penerima: ${recipientName.trim()}${recipientPhone ? ` / ${recipientPhone.trim()}` : ''}] ${finalAddress}`;
      }
        ...
        await supabase.from('sales').insert([
          {
            ...
            payment_method: paymentMethod,
            status: 'pending',
            notes: `Online Order ${customerName} (${deliveryType}) - Jadwal: ${finalScheduleText}`,
          }
        ]);
      } catch (err) {
        // non-blocking
      ...
      openWhatsAppOrder({
        ...
        deliveryType,
        deliveryAddress: deliveryAddress.trim() || STORE_ADDRESS,
        recipientName: isDifferentRecipient ? recipientName.trim() : undefined,
        recipientPhone: isDifferentRecipient ? recipientPhone.trim() : undefined,
        isDifferentRecipient,
        deliverySchedule: finalScheduleText,
        paymentMethod,
        items,
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4">
      <div className="relative w-full max-w-lg bg-white rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200 border border-slate-100">
        ...
        <div className="bg-emerald-600 text-white p-4 sm:p-5 flex items-center justify-between shadow-xs">
          <div>
            ...
            <p className="text-xs text-emerald-100">
              Lengkapi data pengantaran, jadwal & pembayaran
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-4 max-h-[82vh] overflow-y-auto">
          ...
          {/* Section 1: Customer Info (Data Pemesan) */}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  className="w-full text-xs sm:text-sm rounded-xl border border-slate-300 px-3 py-2 focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500"
                />
              </div>
          ...
          {/* Section 2: Opsi Layanan & Alamat Pengiriman */}
          <div className="space-y-3 pt-2 border-t border-slate-100">
            <h3 className="text-xs font-bold text-slate-800 uppercase tracking-wider flex items-center gap-1.5">
              <Truck className="w-3.5 h-3.5 text-emerald-600" />
              <span>2. Opsi Pengambilan / Layanan</span>
            </h3>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                ...
                <div>
                  ...
                  <div className="text-[10px] text-slate-500">Sebrang Masjid Al-Falah</div>
                </div>
              </button>
            </div>

            {/* Address Input & Different Recipient Switch */}
            {deliveryType === 'DELIVERY' ? (
              <div className="mt-2 space-y-3">
                {/* Toggle: Alamat/Penerima Berbeda dengan Pemesan */}
                <div className="p-3 bg-amber-50/70 border border-amber-200/80 rounded-2xl">
                  <label className="flex items-center gap-2.5 cursor-pointer select-none">
                    <input
                      type="checkbox"
                      id="checkbox-different-recipient"
                      checked={isDifferentRecipient}
                      onChange={(e) => setIsDifferentRecipient(e.target.checked)}
                      className="w-4 h-4 text-emerald-600 rounded-md border-amber-300 focus:ring-emerald-500"
                    />
                    <div className="text-xs">
                      <span className="font-bold text-slate-800">
                        Kirim untuk orang lain / Alamat penerima berbeda?
                      </span>
                      <p className="text-[10px] text-slate-500">
                        Centang jika Anda memesankan untuk orang tua, tetangga, atau kerabat
                      </p>
                    </div>
                  </label>

                  {/* Form Nama & HP Penerima Berbeda */}
                  {isDifferentRecipient && (
                    <div className="mt-3 pt-3 border-t border-amber-200/80 grid grid-cols-1 sm:grid-cols-2 gap-2.5 animate-in fade-in duration-200">
                      <div>
                        <label className="block text-[11px] font-bold text-slate-700 mb-1">
                          Nama Penerima di Lokasi <span className="text-red-500">*</span>
                        </label>
                        <input
                          type="text"
                          required={isDifferentRecipient}
                          placeholder="Contoh: Nenek Rodiah / Pak RT"
                          value={recipientName}
                          onChange={(e) => setRecipientName(e.target.value)}
                          className="w-full text-xs rounded-xl border border-slate-300 px-3 py-2 focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500 bg-white"
                        />
                      </div>
                      <div>
                        <label className="block text-[11px] font-bold text-slate-700 mb-1">
                          No. HP / WA Penerima (Opsional)
                        </label>
                        <input
                          type="tel"
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-700 mb-1 flex items-center justify-between">
                    <span className="flex items-center gap-1">
                      ...
                      <span>Alamat Lengkap Pengantaran <span className="text-red-500">*</span></span>
                    </span>
                    <span className="text-[10px] text-slate-400 font-normal">Patokan/RT/RW</span>
                  </label>
                  <textarea
                    id="checkout-address"
                    required
                    rows={2}
                    placeholder="Contoh: Jl. Kalapanunggal RT 02 / RW 01, Sindangkasih, dekat pos kamling atau seberang warung kopi..."
          ...
          {/* Section 3: Jadwal Pengantaran / Pengambilan */}
          <div className="space-y-3 pt-2 border-t border-slate-100">
            <h3 className="text-xs font-bold text-slate-800 uppercase tracking-wider flex items-center gap-1.5">
              <Clock className="w-3.5 h-3.5 text-emerald-600" />
              <span>3. Jadwal {deliveryType === 'DELIVERY' ? 'Pengantaran' : 'Pengambilan'}</span>
            </h3>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              >
                <div className={`w-3 h-3 rounded-full border flex items-center justify-center ${scheduleType === 'ASAP' ? 'border-emerald-600 bg-emerald-600' : 'border-slate-300'}`}>
                  {scheduleType === 'ASAP' && <div className="w-1.5 h-1.5 bg-white rounded-full" />}
                </div>
                <span>⚡ Secepatnya (Langsung)</span>
              </button>
              ...
              >
                <div className={`w-3 h-3 rounded-full border flex items-center justify-center ${scheduleType === 'SCHEDULED' ? 'border-emerald-600 bg-emerald-600' : 'border-slate-300'}`}>
                  {scheduleType === 'SCHEDULED' && <div className="w-1.5 h-1.5 bg-white rounded-full" />}
                </div>
                <span>🕒 Pilih Waktu / Jam</span>
              </button>
            </div>

            {/* Schedule Slot Selection */}
            {scheduleType === 'SCHEDULED' && (
              <div className="space-y-2 p-3 bg-slate-50 rounded-2xl border border-slate-200 animate-in fade-in duration-200">
                <label className="block text-xs font-semibold text-slate-700">
                  Pilih Perkiraan Waktu {deliveryType === 'DELIVERY' ? 'Pengantaran' : 'Ambil'}:
                </label>
                <select
                  value={scheduleTime}
                  onChange={(e) => setScheduleTime(e.target.value)}
                  className="w-full text-xs rounded-xl border border-slate-300 px-3 py-2 bg-white focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500 font-medium text-slate-800"
                >
                  <option value="Pagi Ini (07:30 - 09:30)">🌅 Pagi (07:30 - 09:30 WIB)</option>
                  <option value="Siang Ini (11:00 - 13:00)">☀️ Siang (11:00 - 13:00 WIB)</option>
                  <option value="Sore Ini (15:30 - 17:30)">🌇 Sore (15:30 - 17:30 WIB)</option>
                  <option value="Malam Ini (18:30 - 20:30)">🌙 Malam (18:30 - 20:30 WIB)</option>
                  <option value="Besok Pagi (07:30 - 09:30)">📅 Besok Pagi (07:30 - 09:30 WIB)</option>
                  <option value="Besok Sore (15:30 - 17:30)">📅 Besok Sore (15:30 - 17:30 WIB)</option>
                </select>

                <input
                  type="text"
                  placeholder="Keterangan jam khusus (misal: tepat jam 16:30 setelah pulang kerja)"
                  value={customScheduleNotes}
                  onChange={(e) => setCustomScheduleNotes(e.target.value)}
                  className="w-full text-xs rounded-xl border border-slate-300 px-3 py-2 bg-white focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500"
                />
              </div>
            )}
          </div>
          ...
          <div className="pt-2 border-t border-slate-100">
            <label className="block text-xs font-semibold text-slate-700 mb-1">
              Catatan Tambahan untuk Kasir / Pengantar (Opsional)
            </label>
            <input
              id="checkout-notes"
              type="text"
              placeholder="Contoh: Tolong pilih minyak yang botol baru / titipkan di pos satpam"
              value={notes}
          ...
          >
            {isSubmitting ? (
              ...
              <>
                <Truck className="w-4 h-4 text-emerald-950 stroke-[2.5]" />
