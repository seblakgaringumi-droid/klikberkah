...
export function generateWhatsAppOrderMessage(payload: WhatsAppOrderPayload): string {
  ...
  text += `*Waktu Pesan:* ${dateStr} WIB\n`;
  text += `*Alamat Toko:* ${STORE_ADDRESS}\n`;
  text += `-----------------------------------------\n`;
  text += `*DATA PEMESAN:*\n`;
  text += `👤 *Nama Pemesan:* ${customerName}\n`;
  text += `📞 *No. WA Pemesan:* ${customerPhone}\n`;
  
  if (deliveryType === 'DELIVERY' && isDifferentRecipient && recipientName) {
    text += `-----------------------------------------\n`;
    text += `*PENERIMA DI LOKASI (BEDA ORANG):*\n`;
    text += `👤 *Nama Penerima:* ${recipientName}\n`;
    if (recipientPhone) {
      text += `📞 *No. HP/WA Penerima:* ${recipientPhone}\n`;
    }
  }

  text += `-----------------------------------------\n`;
  text += `*PENGIRIMAN & JADWAL:*\n`;
  text += `📦 *Layanan:* ${serviceLabel}\n`;
  if (deliverySchedule && deliverySchedule.trim()) {
    text += `⏰ *Jadwal Pengantaran/Ambil:* ${deliverySchedule}\n`;
  }
  if (deliveryType === 'DELIVERY') {
    text += `📍 *Alamat Pengiriman:* ${deliveryAddress}\n`;
  } else {
