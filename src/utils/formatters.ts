...
export function formatStock(stock: number, unit: string): string {
  ...
  if (isKg) {
    ...
    if (fracStr) return fracStr.endsWith('gr') ? fracStr : `${fracStr} kg`;
    return `${whole} kg`;
  }
...
export function formatQty(qty: number, unit: string): string {
  ...
  if (isKg) {
    ...
    if (whole > 0 && fracStr) {
      return fracStr.endsWith('gr') ? `${whole} kg ${fracStr}` : `${whole} ${fracStr} kg`;
    }
    if (fracStr) {
      return fracStr.endsWith('gr') ? fracStr : `${fracStr} kg`;
    }
    return `${whole} kg`;
  }
